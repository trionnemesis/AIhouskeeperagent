...
def run_crime(db: str, url: str, county: str, region: str) -> int:
    rows = ps_ingest.ingest_crime(ps_ingest.live_fetch_crime, url)
    stats = ps_ingest.aggregate_crime(rows, county, region)  # DI-5：區域級聚合
    conn = store.connect(db)
    store.init_schema(conn)
    added = store.upsert_crime_area_stats(conn, region, stats, source=_CRIME_SOURCE,
                                          as_of=str(datetime.date.today().year), ingested_at=_now_iso())
    conn.close()
    print(f"[crime] {county}{region} categories={len(stats)} added={added}")
    return added
...
def main() -> None:
    ...
    ap_c.add_argument("--county", required=True)
    ap_c.add_argument("--region", required=True)
    args = ap.parse_args()
    if args.cmd == "lvr":
